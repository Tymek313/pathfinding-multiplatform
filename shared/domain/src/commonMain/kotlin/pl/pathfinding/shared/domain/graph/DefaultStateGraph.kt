...
class DefaultStateGraph(private val graph: Graph) : StateGraph, Graph by graph {
    ...
    override fun createSnapshot(): StateGraph.Snapshot = Snapshot(_nodeStates.toMutableMap())

    override fun restoreFromSnapshot(snapshot: StateGraph.Snapshot) {
        (snapshot as Snapshot).restore(this)
    }

    class Snapshot(val nodeStates: Map<NodeId, NodeState>) : StateGraph.Snapshot {
        ...
        companion object {
            ...
            fun createFromSerialized(values: List<Any>) = Snapshot(values[0] as Map<NodeId, NodeState>)
        }
    }
}